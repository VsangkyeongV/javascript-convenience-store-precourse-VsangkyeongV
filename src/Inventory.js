...
export default class Inventory {
    ...
    buy(INPUT_OBJECTS) {
        ...
        const BUY_ARRAY = this.promotion(INPUT_OBJECTS)
        let BILL = this.buyProcess(BUY_ARRAY)
    ...
    buyProcess(...BUY_ARRAY) {
        ...
        BUY_ARRAY.forEach(buyObject => {
            const SOLD_PRODUCT_INDEX = this.products.findIndex(object => object.name == buyObject[0].name && object.promotion == buyObject[0].promotion)//인덱스찾기
            console.log(SOLD_PRODUCT_INDEX)
            this.products[SOLD_PRODUCT_INDEX].quantity -= Number(buyObject.sell) //팔기
    ...
    //     if (result) {
    //         const IS_RE_PURCHASE = handleNonPromotionProcess(result.name, result.nonPromotion)
    //         if (IS_RE_PURCHASE) {//Y
    //             buyArr.push(result)
    //         } else {
    ...
    promotionResponseCheck(INPUT_OBJECT, result, buyArr) {
        ...
        if (Array.isArray(result)) {
            const NONPROMOTION_OBJECT_INDEX = result.findIndex(object => object.nonPromotion);
            if (NONPROMOTION_OBJECT_INDEX !== -1) {
                const NONPROMOTION_OBJECT = result[NONPROMOTION_OBJECT_INDEX];
                const IS_RE_PURCHASE = handleNonPromotionProcess(NONPROMOTION_OBJECT.name, NONPROMOTION_OBJECT.nonPromotion);
                if (IS_RE_PURCHASE) { // 재구매 시
                    buyArr.push(NONPROMOTION_OBJECT);
                } else { // 비프로모션 객체를 제거 후 result 배열에 저장
                    result.splice(NONPROMOTION_OBJECT_INDEX, 1);
                    buyArr.push(...result);
                }
            } else {
                buyArr.push(...result);
        ...
        // result가 객체일 경우
            ...
            if (result.nonPromotion) { // 비프로모션 속성이 있을 때
                const IS_RE_PURCHASE = handleNonPromotionProcess(result.name, result.nonPromotion);
                if (IS_RE_PURCHASE) { // 재구매 시
                    buyArr.push(result);
                } else { // 재구매하지 않을 시 비프로모션 객체는 제거하지 않음
                    buyArr.push(result);
                }
            } else if (result.needMore) { // 추가 수량이 필요한 경우
                const IS_RE_PURCHASE = handlePromotionProcess(result.name, result.needMore);
                if (IS_RE_PURCHASE) { // 재구매 시
                    INPUT_OBJECT.quantity += Number(result.needMore);
                } else { // 재구매하지 않으면 기존 구매대로 진행
                    result.needMore = undefined;
                    buyArr.push(result);
                }
            } else { // 필요 수량이 충족된 경우
                buyArr.push(result);
